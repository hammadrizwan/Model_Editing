...
def generate_results(model,dataset_paired_train,dataset_paired_test,path_to_folder,mode,comparison,device,loss_function):
    device="cpu"
    if(comparison=="dist"):
        initial_threhold=10
    else:
    ...
    if(loss_function=="triplet"):
        data_loader_test_temp=dp.get_data_loader_triplet(dataset_paired_train,batch_size=1,shuffle=False,device=device)
        distances_sims,predictions_sim,threshold_predictions,targets=rp.get_model_results_triplet(model,data_loader_test_temp,threshold=initial_threhold)
    else:
    ...
    print("threshold_predictions",len(thresholds_pr))
    ...
    print("classfication report train end")
    print("classfication report test")
    
    if(loss_function=="triplet"):
        test_data_loader=dp.get_data_loader_triplet(dataset_paired_test,batch_size=1,shuffle=False,device=device)
        distances_sims,predictions_sim,threshold_predictions,targets=rp.get_model_results_triplet(model,test_data_loader,threshold=initial_threhold)
    else:
        test_data_loader=dp.get_data_loader(dataset_paired_test,batch_size=1,shuffle=False,device=device)
        distances_sims,predictions_sim,threshold_predictions,targets=rp.get_model_results(model,test_data_loader,threshold=best_threshold,comparison=comparison,mode=mode)
    ...
    print("classfication report test end")
    print("writing results to file test and train")
    file_path=path_to_folder+"/results_testset.jsonl"
    rp.write_result_to_file(model,dataset_paired_test,comparison,mode,file_path,device,loss)
    file_path=path_to_folder+"/results_trainset.jsonl"
    rp.write_result_to_file(model,dataset_paired_train,comparison,mode,file_path,device,loss)
    ...
    # print("writing results to file test and train completed")
...
if __name__ == "__main__":
    ...
    # dataset_paired_train,dataset_paired_test=dp.create_dataset_tripletloss(dataset,control)
   
    # print(dataset_paired_train[0][-4:])
    dataset_paired_train=dataset_paired_train+dp.data_construct_high_sim(dataset_paired_train,neightbour_control=0,label_reversal=label_reversal,comparison="sim",topk_neg=15,topk_pos=0,loss=loss)
    print(len(dataset_paired_train))
    # for data in dataset_paired_train:
    ...
    model,mode,comparison,file_path=trainer.train_control(dataset_paired_train=dataset_paired_train,loss_function=loss,
            learning_rate=0.002,weight_decay=0.01,path_to_folder=path_to_folder,epochs=num_epochs,cls_weights=False,device=device,early_stop_patience=early_stop_patience)#cosine, cosine_crossentropy, contrastive
    path_to_folder="./Contrastive_Learning_Automated_AVG_EMB/"+loss+"_mode_"+str(num_samples)+"_"+str(control)+"/"#+"sim_groupings"#+"projector_gaussian"#
   
    print(file_path)
    print("Training Completed")
    print("Result Generation Start")
    generate_results(model,dataset_paired_train,dataset_paired_test,path_to_folder,mode,comparison,device,loss)
