...
from loss_functions import WeightedContrastiveLoss,CosineSimilarityLoss_SentenceTransformers,ContrastiveLoss
...
def train_control(dataset_paired_train,loss_function,learning_rate=0.0001,weight_decay=0.01,path_to_folder="./",epochs=50,cls_weights=True,device="cpu",early_stop_patience=5):
  ...
  if(loss_function=="cosine" or  loss_function=="cosine_crossentropy"):
    ...
    comparison="dist"
    #12.0=MARGIN
    criterion_similarity=WeightedContrastiveLoss(margin=40.0, positive_weight=class_weights[1], negative_weight=class_weights[0]).to(device)
    criterion_classification=None
...
def train_model_triplet(model,optimizer,data_loader,criterion,path_to_folder="./",file_name="best_model_weights.pth"):
    num_epochs = 50
    lowest_error = float('inf')
    best_model_weights = None
